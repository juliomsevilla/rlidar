...
El escaneo del terreno se puede realizar de muy diversas formas y generalmente se adapta a las características del mismo. Por otro lado, la cantidad de puntos que obtenemos del terreno, es relación directa de la velocidad de vuelo, altura de vuelo respecto al terreno y velocidad de emisión-recepción del escaner y se denomina densidad de escaneo, en términos de puntos/m^2^ del terreno.[^lidar-1]

[^lidar-1]: 2 pts/m^2^ implica que en un 95% de las veces, si escogemos aleatoriamente un metro cuadrado del terreno, nos encontraremos al menos 2 retornos
...
![Fuente: esri.com](https://raw.githubusercontent.com/juliomsevilla/rlidar/main/images/ESRI_multiretorno.png "Fuente: esri.com")
...
Con todo lo anterior, uan vez ejecutemos un escaneo LiDAR del terreno, en concreto con un medio aerotransportado, obtendremos, un fichero que generalmente es de extensión `*.las` o ya cada vez de forma más frecuente `*.laz` (a grandes rasgos: fichero original comprimido mediante el algoritmo `zip`) que contendrá al menos los siguientes datos por retorno:
